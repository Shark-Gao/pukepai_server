...
const webSocketShuangjianPlayCard_1 = require("./webSocketShuangjianPlayCard");
...
class webSocketPlayCardRouter {
    ...
    static async robotPlay(userId, roomId) {
        ...
        // 查询最近一条的出牌记录
        const lastRecord = this.getLastRecord(roomId);
        const isFreePlay = !(lastRecord === null || lastRecord === void 0 ? void 0 : lastRecord.userId) || (lastRecord === null || lastRecord === void 0 ? void 0 : lastRecord.userId) == userId;
        const aiStrategy = (0, doudizhuAi_1.getDoudizhuAiStrategy)(roomInfo.robot_level);
    ...
    static async gameOver(roomId, userId) {
        ...
        const [rows] = await mysql_1.default.inst.query(`select user_id, gold, user_account from users where user_id in ("${roomInfo.roomUserIdList[0]}","${roomInfo.roomUserIdList[1]}","${roomInfo.roomUserIdList[2]}")`);
        ...
        if (successUser.user_id) {
            ...
            const promiseList = [...successUserGold, ...loseUserGold].map(item => {
                return new Promise(async (resolve, reject) => {
                    ...
                    let [rows] = await mysql_1.default.inst.query(`update users set gold = ? where user_id = ? `, [item.gold, item.user_id]);
    ...
    async userPlayCard({ ws, token, userInfo, params }) {
        ...
        if (roomInfo && ((_a = roomInfo.game_mode) !== null && _a !== void 0 ? _a : IGameMode_1.GameMode.DOUDIZHU) === IGameMode_1.GameMode.SHUANGJIAN) {
            await (0, webSocketShuangjianPlayCard_1.handleShuangjianUserPlayCard)({ ws, token, userInfo, params });
            return;
        }
        const roomUserIdList = roomInfo.roomUserIdList;
...
//# sourceMappingURL=webSocketPlayCardRouter.js.map