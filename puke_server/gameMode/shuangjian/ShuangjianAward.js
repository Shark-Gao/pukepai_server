...
exports.calcAwardsForUser = calcAwardsForUser;
exports.calcShuangjianSettlement = calcShuangjianSettlement;
/**
 ...
 */
const ShuangjianMode_1 = require("./ShuangjianMode");
const HEAD_AWARDS_DEFAULT = {
    4: 1, // 4-head: 4 of a same suit -> 1 award
    7: 4, // 7-head -> 4 awards
    8: 6, // 8-head -> 6 awards
};
const FIVE_TEN_K_AWARDS = {
    1: 0, 2: 0,
 ...
 */
function detectHeads(cards) {
    // Group by [rank, suit] for a same-suit-aware 4-head check.
    const bySuitRank = {};
    const byRank = {};
    for (const c of cards) {
        const real = (0, ShuangjianMode_1.toRealCard)(c);
        if (real === 53 || real === 54)
            continue;
    ...
    // with extra pairs.
    const triples = Object.keys(byRank).map(Number).filter(r => byRank[r] === 3);
    const pairs = Object.keys(byRank).map(Number).filter(r => byRank[r] >= 2 && byRank[r] !== 3);
    if (triples.length >= 2) {
        // 3+3 pairs
...
function detect510KCount(cards) {
    const fives = [];
    const tens = [];
    const kings = [];
    for (const c of cards) {
...
function calcAwardsForUser(cards) {
    var _a;
    ...
    const five10KAwards = (_a = FIVE_TEN_K_AWARDS[five10K]) !== null && _a !== void 0 ? _a : 0;
 ...
 */
function calcShuangjianSettlement(input) {
    var _a;
    const { isBaopai, drawAsOne, doubleScore, fiveAwardChallenge, rankList, landlordCamp, farmerCamp, bankerId, baseScore } = input;
    // Map userId -> rank
    const rankMap = {};
    ...
    if (isBaopai) {
        ...
        if (bankerRank === 1) {
            victoryStatus = 'baopai-win';
            multiplier = 6;
        }
        else {
            victoryStatus = 'baopai-lose';
            multiplier = 6;
        } // pay banker
    }
    else {
    ...
    for (const uid of allUsers) {
        ...
        const camp = inLandlord ? 'landlord' : 'farmer';
        let getScore = 0;
        ...
        const myAwards = ((_a = rankList.find(r => r.userId === uid)) === null || _a === void 0 ? void 0 : _a.awards) || 0;
        if (fiveAwardChallenge && myAwards >= 5) {
            ...
            for (const winnerUid in fiveAwardExtra) {
                if (winnerUid !== uid)
                    getScore -= fiveAwardExtra[winnerUid];
            }
        }
        results.push({
            userId: uid,
...
//# sourceMappingURL=ShuangjianAward.js.map