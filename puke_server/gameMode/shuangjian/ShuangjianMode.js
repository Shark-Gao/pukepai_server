...
exports.ShuangjianMode = void 0;
exports.toRealCard = toRealCard;
/*
 * @author: sharkgao
...
const ShuangjianCardLogic_1 = require("./ShuangjianCardLogic");
const ShuangjianAward_1 = require("./ShuangjianAward");
const shuangjianRecord_1 = require("../../mysql/shuangjianRecord");
/** Convert a Shuangjian double-deck card id back to the canonical 1..54 id. */
function toRealCard(id) {
...
class ShuangjianMode extends IGameMode_1.IGameMode {
    // ----- Capability -----
    /** Fengcheng Shuangjian is a fixed 4-seat game. */
    getMaxPlayerCount() { return 4; }
    getMinPlayerCount() { return 4; }
    // ----- Lifecycle -----
    /**
    ...
    dealCards(roomInfo) {
        ...
        for (const uid of userIds) {
            ...
            const newRoomUsers = (0, tools_1.clientReturnRoomUsers)(roomInfo.roomUsers, uid);
    ...
    selectLandlord(roomInfo) {
        const userIds = roomInfo.roomUserIdList.filter((id) => !!id);
        let bankerId;
        const lastHead = roomInfo._last_head_user_id || '';
        if (lastHead && userIds.indexOf(lastHead) >= 0) {
            bankerId = lastHead;
        }
        else {
            bankerId = userIds[(0, tools_1.getRandomNumber)(0, userIds.length - 1)];
        }
        roomInfo.landlord_id = bankerId;
        // Broadcast banker
        for (const uid of userIds) {
            (0, webSocket_1.wsSend)(roomInfo.roomUsers[uid].ws, {
     ...
     */
    onAfterLandlordDecided(roomInfo) {
    ...
    applyBaopaiResult(roomInfo, isBao) {
        ...
        if (isBao) {
            ...
            roomInfo.farmer_camp = userIds.filter((id) => id !== bankerId);
            ...
            for (const uid of userIds) {
                if (uid === bankerId)
                    continue;
                const hand = (roomInfo.roomUsers[uid].user_card || []);
                for (const c of hand)
                    candidatePool.push(c);
            }
            if (candidatePool.length > 0) {
                const idx = (0, tools_1.getRandomNumber)(0, candidatePool.length - 1);
                roomInfo.partner_card = candidatePool[idx];
                const partnerUserId = userIds.find((uid) => {
                    if (uid === bankerId)
                        return false;
                    const hand = (roomInfo.roomUsers[uid].user_card || []);
                ...
                roomInfo.farmer_camp = userIds.filter((id) => roomInfo.landlord_camp.indexOf(id) < 0);
                roomInfo.partner_revealed = true;
            }
            else {
    ...
    judgeCardType(cards) {
        const r = (0, ShuangjianCardLogic_1.judgeCardTypeShuangjian)(cards);
        return {
            valid: r.valid,
            cardType: r.type,
            weight: r.mainRank,
    ...
    compareCards(a, b) {
        if (!a || !b || !a.extra || !b.extra)
            return 0;
        return (0, ShuangjianCardLogic_1.compareShuangjian)(a.extra, b.extra);
    }
    getCardHint(targetCards, myCards) {
        return (0, ShuangjianCardLogic_1.cardHintShuangjian)(targetCards || [], myCards || []);
    }
    // ----- Play hooks -----
    /**
     ...
     */
    onPlayCard(roomInfo, userId, cards) {
        ...
        if (!roomInfo.is_baopai && !roomInfo.partner_revealed && roomInfo.partner_card > 0) {
            if (cards.indexOf(roomInfo.partner_card) >= 0) {
                ...
                roomInfo.farmer_camp = roomInfo.roomUserIdList
        ...
        if (me && (!me.user_card || me.user_card.length === 0)) {
            const already = roomInfo.pass_user_record.some((r) => r.userId === userId);
            if (!already) {
    ...
    isGameOver(roomInfo) {
        ...
        const lc = roomInfo.landlord_camp || [];
        const fc = roomInfo.farmer_camp || [];
        if (roomInfo.partner_revealed) {
            if (lc.length >= 2 && lc.every(isEmpty))
                return true;
            if (fc.length >= 2 && fc.every(isEmpty))
                return true;
            return false;
        }
    ...
    calcSettlement(roomInfo) {
        ...
        const finished = roomInfo.pass_user_record
        ...
        const rankList = orderedIds.map(uid => {
            const detail = (0, ShuangjianAward_1.calcAwardsForUser)(startCards[uid] || []);
            awardMap[uid] = detail;
            return { userId: uid, awards: detail.totalAwards };
        });
        const { victoryStatus, results } = (0, ShuangjianAward_1.calcShuangjianSettlement)({
        ...
        roomInfo._shuangjian_victory_status = victoryStatus;
    ...
    async saveRecordMysql(roomInfo, settlement) {
        ...
        await (0, shuangjianRecord_1.saveShuangjianRecordMysql)(roomInfo);
    }
}
exports.ShuangjianMode = ShuangjianMode;
//# sourceMappingURL=ShuangjianMode.js.map