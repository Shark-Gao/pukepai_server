...
exports.SjCardType = void 0;
exports.judgeCardTypeShuangjian = judgeCardTypeShuangjian;
exports.compareShuangjian = compareShuangjian;
exports.cardHintShuangjian = cardHintShuangjian;
/**
...
var SjCardType;
(function (SjCardType) {
    SjCardType[SjCardType["INVALID"] = 0] = "INVALID";
    SjCardType[SjCardType["SINGLE"] = 1] = "SINGLE";
    SjCardType[SjCardType["PAIR"] = 2] = "PAIR";
    SjCardType[SjCardType["THREE_WITH_TWO"] = 3] = "THREE_WITH_TWO";
    SjCardType[SjCardType["PLANE"] = 4] = "PLANE";
    SjCardType[SjCardType["STRAIGHT"] = 5] = "STRAIGHT";
    SjCardType[SjCardType["DOUBLE_STRAIGHT"] = 6] = "DOUBLE_STRAIGHT";
    SjCardType[SjCardType["FIVE_TEN_K"] = 7] = "FIVE_TEN_K";
    SjCardType[SjCardType["BOMB"] = 8] = "BOMB";
    SjCardType[SjCardType["KING_BOMB"] = 9] = "KING_BOMB";
})(SjCardType || (exports.SjCardType = SjCardType = {}));
/** Shuangjian rank ordering, big→small (3 is smallest, joker biggest). */
const SJ_RANK_ORDER = [54, 53, 2, 1, 13, 12, 11, 10, 9, 8, 7, 6, 5, 4, 3];
/** Get the rank index (0=biggest, 14=smallest). 53/54 are kings. */
function rankIdx(realCard) {
    if (realCard === 53 || realCard === 54)
        return SJ_RANK_ORDER.indexOf(realCard);
    const v = (realCard - 1) % 13 + 1;
    return SJ_RANK_ORDER.indexOf(v);
}
/** Get rank value (1..13 plus 53/54 for jokers). */
function rankValue(realCard) {
    if (realCard === 53 || realCard === 54)
        return realCard;
    return (realCard - 1) % 13 + 1;
}
/** Suit: 0 diamond / 1 club / 2 heart / 3 spade / 4 joker. */
function suit(realCard) {
    if (realCard === 53 || realCard === 54)
        return 4;
...
function countByRank(cards) {
    ...
    for (const c of cards) {
        const r = rankValue((0, ShuangjianMode_1.toRealCard)(c));
        map[r] = (map[r] || 0) + 1;
    }
    return map;
}
 ...
 */
function detect510K(cards) {
    if (cards.length !== 3)
        return { valid: false, suited: false };
    const ranks = cards.map(c => rankValue((0, ShuangjianMode_1.toRealCard)(c))).sort((a, b) => a - b);
    // 5,10,13(K)
    if (ranks[0] !== 5 || ranks[1] !== 10 || ranks[2] !== 13)
        return { valid: false, suited: false };
    const suits = cards.map(c => suit((0, ShuangjianMode_1.toRealCard)(c)));
 ...
 * { count, allSuited }.
 */
function detectMulti510K(cards) {
    ...
    const fives = cards.filter(c => rankValue((0, ShuangjianMode_1.toRealCard)(c)) === 5).map(c => suit((0, ShuangjianMode_1.toRealCard)(c)));
    const tens = cards.filter(c => rankValue((0, ShuangjianMode_1.toRealCard)(c)) === 10).map(c => suit((0, ShuangjianMode_1.toRealCard)(c)));
    const kings = cards.filter(c => rankValue((0, ShuangjianMode_1.toRealCard)(c)) === 13).map(c => suit((0, ShuangjianMode_1.toRealCard)(c)));
    let suitedAll = true;
    fives.sort();
    tens.sort();
    kings.sort();
...
function detectBombOrKingBomb(cards) {
...
function detectStraight(cards) {
    ...
    if (!ranks.every(r => map[r] === 1))
        return null;
    if (ranks.some(r => r === 2 || r === 53 || r === 54))
        return null;
    ...
    for (let i = 1; i < sorted.length; i++) {
        if (orderedAsc.indexOf(sorted[i]) - orderedAsc.indexOf(sorted[i - 1]) !== 1)
            return null;
    }
    return {
 ...
 */
function detectThreeWithTwoOrPlane(cards) {
    if (cards.length < 5)
        return null;
    const map = countByRank(cards);
 ...
 * stronger combo.
 */
function judgeCardTypeShuangjian(cards) {
    if (!cards || cards.length === 0)
        return INVALID;
    // 1) King bomb
    const kb = detectBombOrKingBomb(cards);
    if (kb && kb.type === SjCardType.KING_BOMB)
        return kb;
    ...
    // 3) Bomb (≥4 same rank)
    if (kb && kb.type === SjCardType.BOMB)
        return kb;
    // 4) Single / Pair / 3+2 / Plane
    if (cards.length === 1) {
        const r = rankValue((0, ShuangjianMode_1.toRealCard)(cards[0]));
        return {
    ...
    if (cards.length === 2) {
        const r0 = rankValue((0, ShuangjianMode_1.toRealCard)(cards[0]));
        const r1 = rankValue((0, ShuangjianMode_1.toRealCard)(cards[1]));
 ...
 */
function categoryWeight(r) {
    if (r.type === SjCardType.FIVE_TEN_K) {
        if (r.fiveTenKCount >= 4)
            return 120;
        if (r.fiveTenKCount === 3)
            return 80;
        // single 510K
 ...
 */
function compareShuangjian(previous, current) {
 ...
 */
function cardHintShuangjian(targetCards, myCards) {
    ...
    if (bombRanks.length > 0) {
        ...
        for (const r of bombRanks) {
            ...
            if (compareShuangjian(targetJ, cj) > 0)
                return candidate;
        }
    }
...
//# sourceMappingURL=ShuangjianCardLogic.js.map